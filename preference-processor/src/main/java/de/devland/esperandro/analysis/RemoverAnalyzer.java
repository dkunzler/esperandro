...
public class RemoverAnalyzer implements GeneratorAwareAnalyzer {
    ...
    public String getPreferenceName(MethodInformation method) {
        ...
        return methodName.substring(0, methodName.length() - Constants.SUFFIX_REMOVE.length());
    }

    @Override
    public TypeInformation getPreferenceType(MethodInformation method) {
        // type of preference cannot be deducted by the remover
        return null;
    }

    @Override
    public MethodOperation getMethodOperation(MethodInformation method) {
        return MethodOperation.REMOVE;
    }

    @Override
    public MethodGenerator getGenerator() {
        return new CollectionActionGenerator("remove");
    }
}
